...
public class PasswordEditText extends TextInputEditText {
	 ...
	 */
	private final static int EXTRA_TAPPABLE_AREA = 80;

	@DrawableRes
	private int showPwIcon = R.drawable.ic_password_normal;

	@DrawableRes
	private int hidePwIcon = R.drawable.ic_password_select;
	...
	private void showPasswordVisibilityIndicator(boolean shouldShowIcon) {
		if (shouldShowIcon) {
			...
			showingIcon = true;
			setCompoundDrawablesRelativeWithIntrinsicBounds(ContextCompat.getDrawable(getContext(), R.drawable.selector_password_icon).mutate(),null,drawable,null);
		} else {
			showingIcon = false;
		}
	}
